...
import { Avatar, Badge, Card } from "@campus-one/ui";
import { dossiers } from "@campus-one/mocks";
            ...
            <tbody className="divide-y divide-line">
              {rows.map((d, i) => (
                <tr key={d.id} className="hover:bg-surface">
                  <td className="px-4 py-3"><div className="flex items-center gap-2.5"><Avatar name={d.candidate} size="sm" /><Link href={`/dossiers/${d.id}`} className="font-medium text-navy-900 hover:text-brand-700">{d.candidate}</Link></div></td>
                  <td className="px-4 py-3"><p>{d.email}</p><p className="text-xs text-muted">{d.phone}</p></td>
                  <td className="px-4 py-3">{d.country}</td>
                  <td className="px-4 py-3"><Link href={`/dossiers/${d.id}`} className="font-mono text-xs text-brand-600 hover:underline">{d.id}</Link>{i % 7 === 0 && <span className="ml-1 text-xs text-muted">+1</span>}</td>
