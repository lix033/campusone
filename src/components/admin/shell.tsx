...
import { Avatar, Dialog, Logo, cn } from "@/components/ui";
import { bordereaux, campaigns, dossiers, roles } from "@/lib/mocks";
import { demoUsers, useRole, useSession } from "@/lib/admin/permissions";
...
const sections: { title: string; items: NavItem[] }[] = [
  {
    ...
    items: [
      ...
      { href: "/admin/dossiers", label: "Dossiers", icon: FolderKanban, perm: "dossiers.read", badge: dossiers.filter((d) => d.docsToReview > 0).length },
      { href: "/admin/candidats", label: "Candidats", icon: Users, perm: "dossiers.read" },
      { href: "/admin/paiements", label: "Paiements", icon: Banknote, perm: "payments.read", badge: bordereaux.length },
      { href: "/admin/rendez-vous", label: "Rendez-vous", icon: CalendarDays, perm: "dossiers.edit" },
    ],
  },
  {
    title: "Configuration",
    items: [
      { href: "/admin/parcours", label: "Parcours d'étapes", icon: Workflow, perm: "config.workflows" },
      { href: "/admin/campagnes", label: "Campagnes", icon: Megaphone, perm: "config.campaigns" },
      { href: "/admin/formations", label: "Catalogue", icon: BookOpen, perm: "config.catalog" },
      { href: "/admin/types-documents", label: "Types de documents", icon: FileStack, perm: "config.catalog" },
      { href: "/admin/partenaires", label: "Partenaires", icon: Handshake, perm: "config.catalog" },
    ],
  },
  {
    title: "Administration",
    items: [
      { href: "/admin/roles", label: "Utilisateurs et rôles", icon: ShieldCheck, perm: "config.roles" },
      { href: "/admin/audit", label: "Journal d'audit", icon: ScrollText, perm: "audit.read" },
    ],
  },
];
...
function GlobalSearch() {
  ...
  React.useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      } else if (g && e.key === "d") router.push("/admin/dossiers");
      else if (g && e.key === "t") router.push("/admin");
    };
  ...
  return (
    <div className="relative w-full max-w-md">
      ...
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" && results[0]) router.push(`/admin/dossiers/${results[0].id}`);
        ...
        <div className="absolute inset-x-0 top-12 z-50 overflow-hidden rounded-xl border border-line bg-white shadow-lift">
          {results.length ? (
            <ul>
              {results.map((d) => (
                <li key={d.id}>
                  <Link href={`/admin/dossiers/${d.id}`} className="flex items-center gap-3 px-3 py-2.5 hover:bg-surface">
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    if (session.ready && !session.role) router.replace(`/admin/connexion?next=${encodeURIComponent(pathname)}`);
  ...
  const brand = (
    <div className="px-6 pb-5 pt-5">
      <Link href="/admin" aria-label="Tableau de bord"><Logo variant="white" suffix="Back-office" /></Link>
    </div>
  );
            ...
            <div className="relative">
              ...
              {menu && (
                <div role="menu" className="absolute right-0 z-50 mt-2 w-72 animate-toast-in rounded-2xl border border-line bg-white p-2 shadow-lift">
                  ...
                  <button onClick={() => { setMenu(false); session.signOut(); router.push("/admin/connexion"); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-navy-800 hover:bg-surface"><LogOut className="size-4" aria-hidden /> Se déconnecter</button>
                </div>
              )}
            </div>
          </div>
