...
import { Avatar, Badge, Button, Card, Dialog, EmptyState, Field, Select, cn, useToast } from "@/components/ui";
import { agents, dossiers, pipelineSteps, type AdminDossier } from "@/lib/mocks";
import { PageHeader } from "./shell";
import { Can, useRole } from "@/lib/admin/permissions";
import { paymentLabel, paymentTone, priorityTone } from "@/lib/admin/labels";
...
export function DossiersTable() {
  ...
  React.useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter" && visible[cursor]) router.push(`/admin/dossiers/${visible[cursor]!.id}`);
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <Link href={`/admin/dossiers/${d.id}`} onClick={(e) => e.stopPropagation()} className="font-semibold text-navy-900 hover:text-brand-700">{d.candidate}</Link>
