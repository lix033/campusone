...
import { Avatar, Badge, Card, CardHeader, Progress, buttonClasses, cn } from "@/components/ui";
import { agents, appointmentRequests, auditLog, bordereaux, dashboardStats, dossiers } from "@/lib/mocks";
import { PageHeader } from "@/components/admin/shell";
import { BarList, Columns } from "@/components/admin/charts";
...
export default function DashboardPage() {
  ...
  const today = [
    { icon: FileSearch, n: docs, label: "pièces à vérifier", sub: `dans ${dossiers.filter((d) => d.docsToReview).length} dossiers`, href: "/admin/dossiers?filtre=documents", tone: "brand", show: can("docs.review") },
    { icon: Landmark, n: bordereaux.length, label: "bordereaux à valider", sub: `dont ${bordereaux.filter((b) => b.sinceHours > 48).length} depuis plus de 48 h`, href: "/admin/paiements", tone: "warning", show: can("payments.validate") },
    { icon: CalendarClock, n: appointmentRequests.length, label: "rendez-vous à confirmer", sub: "cette semaine", href: "/admin/rendez-vous", tone: "info", show: can("dossiers.edit") },
    { icon: Timer, n: deadlines, label: "échéances sous 30 jours", sub: "Campus France et visa", href: "/admin/dossiers?filtre=echeances", tone: "danger", show: can("dossiers.read") },
  ].filter((x) => x.show);
  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        ...
        <Card>
          <CardHeader title="Activité récente" action={can("audit.read") ? <Link href="/admin/audit" className="text-sm font-semibold text-brand-600">Journal</Link> : undefined} />
      ...
      <div className="mt-6 flex justify-end">
        <Link href="/admin/dossiers" className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-600 hover:text-brand-700">Tous les dossiers <ArrowRight className="size-4" aria-hidden /></Link>
      </div>
    </>
  );
}
